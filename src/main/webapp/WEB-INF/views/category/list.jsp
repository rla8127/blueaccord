...
<div class="container">
    <div class="category-container">
        <div class="category-header">
            <h2>${category}</h2>
            <c:if test="${category ne 'BEST'}">
                <p>총 ${totalCnt}개의 상품이 있습니다.</p>
            </c:if>
            <c:if test="${category eq 'BEST'}">
                <p><strong>판매량 상위 BEST 30</strong></p>
            </c:if>
        </div>
        <div class="sort-container">
            <form id="sort-form" action="<c:url value="/category"/>" class="sort-form" method="get">
                <input type="hidden" class="search-category" name="category" value="${category}"/>
                <div>
                    <c:if test="${category ne 'BEST' && category ne 'NEW'}">
        ...
        <div class="paging">
          ...
          <c:if test="${totalCnt!=null && totalCnt!=0}">
            <c:if test="${ph.showPrev}">
              <a class="page" href="<c:url value="/category${ph.sc.getQueryString(ph.beginPage-1)}"/>">&lt;</a>
            </c:if>
            <c:forEach var="i" begin="${ph.beginPage}" end="${ph.endPage}">
              <a class="page ${i==ph.sc.page? "paging-active" : ""}" href="<c:url value="/category${ph.sc.getQueryString(i)}"/>">${i}</a>
            </c:forEach>
            <c:if test="${ph.showNext}">
              <a class="page" href="<c:url value="/category${ph.sc.getQueryString(ph.endPage+1)}"/>">&gt;</a>
            </c:if>
          </c:if>
        </div>
      </div>
